...
def login():
    ...
    body={'fmdo':'login','dopost':'login','userid':username,'pwd':password}
    ...
    DedeUserIDckMd5=re.compile('DedeUserIDckMd5=[\w\d]+;').findall(setcookie)[1]
    DedeLoginTime=re.compile('DedeLoginTime=\d+;').findall(setcookie)[0]
    DedeLoginTimeckMd5=re.compile('DedeLoginTimeckMd5=[\w\d]+;').findall(setcookie)[1]
    cookie=PHPSESSID+DedeUserID+DedeUserIDckMd5+DedeLoginTime+DedeLoginTimeckMd5+'AJSTAT_ok_pages=6; AJSTAT_ok_times=1; bdshare_firstime=1372830511943;acopendivids=jason; acgroupswithpersist=pets'
...
def go(catUrl,catName):
	...
	last_page_found=True
    	...
    	try:
    		for i in range(1000):
			j=i+1
			url=catUrl.replace('.html',str(j)+".html")
			...
			entries=re.compile('<input name="id\[\]" type="checkbox" value="(\d+)"  onclick="getcheck\(\'id\[\]\'\)"/><a href="[/\w\d]*.html" target="_blank">([^<>"]*)</a>').findall(html)
			if(len(entries)==0):
				print '[End]'+url
				break;
			...
			for entry in entries:
				aId=entry[0]
				title=entry[1]
				mp3path=catName.decode('utf-8')+"/"+title+".mp3"
...
catList=[["http://www.baobao88.com/list/101/15--182--136--.html",'7-12岁/国外作品/生活小说'],['']]
